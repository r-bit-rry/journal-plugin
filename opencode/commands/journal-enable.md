...
1b. **Check for legacy location (git repos only):**
    - If this is a git repo AND `journal/manifest.md` exists AND the git-backed journal manifest does NOT exist:
      ...
      - If no: this is the **legacy_keep_existing** path (stop_after: `legacy_kept`). Resolve journal dir to `journal/`. The assistant reply must be **only** the short confirmation: "Migration skipped. Keeping existing journal data in `journal/`." Stop — do not run the wizard, and do not modify `manifest.md`, `AGENTS.md`, or `.gitignore`
...
6. Check if `AGENTS.md` exists in project root:
   - If `AGENTS.md` exists, read its contents first — only append the journal section if no `Journal Plugin` section already exists
   - If it does not exist, create `AGENTS.md` with the journal reference
   - If the user works from multiple linked worktrees, also mention that the same snippet in `~/.config/opencode/AGENTS.md` keeps startup continuity aligned with the shared git-backed journal
   ...
   Run `/log` to capture factual session notes.
   ```
...
7. Confirm to user that journal has been enabled and explain:
   ...
   - Context from past journals will be loaded on session start via the project `AGENTS.md` `journal-context` skill snippet
   - If the user uses linked worktrees, recommend adding the same snippet to `~/.config/opencode/AGENTS.md` so startup continuity follows the shared git-backed journal across worktrees
