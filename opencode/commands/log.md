---
description: Capture factual session notes into the daily journal entry
---
...
3. **Extract notes from the current conversation:**
   ...
   - **Summary**: One short factual line describing the main outcome
   - When the session is preparatory (e.g. compaction snapshots, planning), frame the summary around what was prepared and for what purpose, not just the actions performed
   ...
   - **Supported Patterns** (optional, only if clearly evidenced):
     - Only include if the same fact pattern appears at least 3 times, the user explicitly confirms it, or 3+ user statements in a single conversation reinforce the same theme (thematic convergence counts toward the 3-occurrence threshold)
   ...
   - **User Notes** preserve the user's own phrasing and voice — do not paraphrase into third person (write "Do not use mock mode" not "Explicitly requested no mock runs")
   - **Facts** mirror the granularity of the source: if an assistant message describes multiple related items in one statement, keep them as one bullet — do not split into separate bullets
   - **Decisions** capture the chosen approach or strategy, not the user's request restated — the decision is *how* (the approach selected), not *what* (the task requested). Prefer one unified decision over several granular sub-steps
